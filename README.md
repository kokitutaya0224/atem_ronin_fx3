...
./setup-mac.sh      # ① このMacのIP と ② ATEMのIP を対話入力
                    #    → USB-LANポートを自動検出し固定IPを設定（要管理者パスワード）
                    #    → ESP32(192.168.10.51) / ATEM への疎通も自動チェック
...
`setup-mac.sh` で聞かれるIP:

| 項目 | 値の例 | 注意 |
|---|---|---|
| このMacのIP | `192.168.10.100` | `192.168.10.x`。`.51〜.70`（ジンバル予約）と `.239`（ATEM）は不可。他のオペレーターと重複しない値 |
| ATEMのIP | `192.168.10.239` | 既定値が `config.json` に入っているので通常はそのままEnter |
