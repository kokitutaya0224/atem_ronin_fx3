...
2回目以降は `./start.sh` だけでよい。サーバーが立ち上がると **このMacの
ブラウザで操作パネルが自動で開く**。ターミナルには他PC用のURLも表示される。

**他のPC・タブレットから操作する**（同一LANにいれば可）:
`./start.sh` が表示する `http://<MacのIP>:8090` を、操作したい端末の
ブラウザで開くだけ。サーバーは `0.0.0.0:8090` で待ち受けているので
追加設定は不要。複数端末を同時に開いてもよい（状態はWebSocketで同期）。
Macと同じ 192.168.10.x（ATEM/ジンバルのLAN）か、Macのもう一方の
ネットワークのどちらでも、その端末が到達できる方のアドレスを使う。
