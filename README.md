...
オペレーターのMacは毎回変わる前提。手順は3ステップ。

### 前提（そのMacで一度だけ）

- **Python 3**（無ければターミナルで `xcode-select --install`）
- **USB-Ethernetアダプタ**（AX88179系）＋ LANケーブルで現場のPoEスイッチへ接続
- リポジトリを clone: `git clone https://github.com/kokitutaya0224/atem_ronin_fx3.git`

### 現場での起動（毎回）

```bash
cd atem_ronin_fx3/bridge

./setup-mac.sh      # ジンバルIP / ATEM IP / このMacのIP を対話入力
                    #    → USB-LANポートを自動検出し固定IPを設定（要管理者パスワード）
                    #    → ジンバル / ATEM への疎通も自動チェック

./start.sh          # 初回はvenv作成も自動。数十秒でブラウザに操作パネルが開く

# 終わって片付けるとき（そのMacをDHCPに戻す・任意）
./reset-mac-net.sh
```

`setup-mac.sh` で聞かれるIP（いずれも `192.168.10.x`、既定値でよければEnter）:

| 項目 | 既定 | 注意 |
|---|---|---|
| ジンバル(ESP32)のIP | `192.168.10.51` | `config.json` に書き込まれる。**ESP32本体のIPはファーム(`firmware/src/config.h` の `STATIC_IP`)で決まる**ので、両者を一致させること |
| ATEMのIP | `192.168.10.239` | `config.json` に書き込まれる |
| このMacのIP | `192.168.10.100` | 有線ポートに設定。ジンバル・ATEMと重複しない値。複数オペレーター間でも重複させない |

**他のPC・タブレットからも操作する**（同一LANにいれば可）:
`./start.sh` が表示する `http://<MacのIP>:8090` を、操作したい端末の
ブラウザで開くだけ。サーバーは `0.0.0.0:8090` で待ち受けている。
複数端末を同時に開いてもよい（状態はWebSocketで同期）。
**ただしブリッジ（`start.sh`）を動かすMacは1台だけにすること。**
複数台で起動するとESP32へのUDPとATEM接続が競合する。
...
3. **CANプロトコル実装はDJI公式文書と照合済み（2026-07-09）**:
   ...
   **✅ 2026-09-10 実機検証完了**: 配線接続状態でRS 4 Proと1MbpsのCAN疎通に成功。
   起動時自己診断で `CAN自己診断OK — Ronin応答` を確認（8/31まで続いた
   `tx_err=128` のバスオフは配線が通って解消）。yaw/pitch/roll の3軸すべてで
   角度取得・速度制御（コマンド5.0deg/s に対し実測4.9deg/s）・位置制御
   （目標±15°に対し誤差0.2°以内）が動作。ウォッチドッグ（400ms）による
   停止も確認。RS 4 Pro は CAN ID 0x426 で状態フレームを高頻度に流すが
   0x222 以外は無視するので影響なし。シリアルポートは環境により変動する
   （今回は `/dev/cu.usbserial-1430`）。Mac の有線LAN（en21）は起動ごとに
   `sudo ifconfig en21 192.168.10.23 255.255.255.0` の再設定が必要。
...
- [x] RoninとのCAN疎通・3軸の速度/位置制御（2026-09-10、UDP直送で検証）
- [ ] パネル/ゲームパッドからRoninのパン・チルトが滑らかに動く（Webパネル経由の通しは未）
